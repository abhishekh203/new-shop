...
const HorizontalProductGallery = ({ title = productGalleryConfig.featuredTitle, category = null }) => {
  ...
  // Limit products for better performance - show max 20 products
  const limitedProducts = productsWithImages.slice(0, 20);
  
  // Duplicate products for infinite scroll effect
  const displayProducts = [...limitedProducts, ...limitedProducts];
  ...
  // Calculate animation distance more efficiently
  const cardWidth = 192; // w-48 = 192px
  const gap = 32; // space-x-8 = 32px
  const totalCardWidth = cardWidth + gap;
  const animationDistance = -(limitedProducts.length * totalCardWidth);
  ...
  return (
    <section className={`py-12 ${serifTheme.gradients.background} overflow-hidden`} style={{ fontFamily: serifTheme.fontFamily.serif }}>
      <div className="max-w-full">
        ...
        {/* Auto-scrolling Products Container - Optimized */}
          ...
          <motion.div
            ...
            transition={{
              x: {
                repeat: Infinity,
                repeatType: "loop",
                duration: limitedProducts.length * 2,
                ease: "linear",
              },
            }}
          ...
          >
            {displayProducts.map((product, index) => (
              <div
                ...
                style={{ willChange: 'transform' }}
              >
                <div className={`${serifTheme.gradients.card} backdrop-blur-xl ${serifTheme.radius.card} overflow-hidden shadow-lg hover:shadow-xl border ${serifTheme.colors.border.primary} hover:border-amber-300/80 transition-all duration-200`}>
                  {/* Product Image */}
