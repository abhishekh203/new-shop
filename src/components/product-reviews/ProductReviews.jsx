...
import { normalizeReview, safeParseJSON } from '../../utils/dataNormalization';
...
const ProductReviews = ({ productId, productTitle, showReviewForm = true, maxDisplayed = null }) => {
    ...
    // User data
    const user = useMemo(() => safeParseJSON('users'), []);
    const userId = user?.id || null;

    // Use the imported normalizeReview function with product context
    const normalizeReviewWithContext = useCallback((item) => {
        const normalized = normalizeReview(item);
        if (normalized) {
            // Add product context if missing
            normalized.productTitle = normalized.productTitle || productTitle;
            normalized.productId = normalized.productId || productId;
            // Add additional fields specific to this component
            normalized.userEmail = item.user_email ?? item.userEmail ?? null;
            normalized.isAnonymousReview = item.is_anonymous_review ?? item.isAnonymousReview ?? !item.user_id;
            normalized.reported = item.reported ?? false;
            normalized.approved = item.approved ?? false;
        }
        return normalized;
    ...
    const fetchProductReviews = useCallback(async () => {
        ...
        setLoading(true);
        try {
            // Prefer context data when available
            let productReviews = [];
            if (Array.isArray(getAllReview) && getAllReview.length > 0) {
                productReviews = getAllReview
                    .filter((item) => (item.productId ?? item.product_id) === productId)
                    .map(normalizeReviewWithContext);
            } else {
                ...
                productReviews = (data || []).map(normalizeReviewWithContext);
            }

            setReviews(productReviews);
        } catch (error) {
            logger.error('Error fetching reviews:', { error: error.message || error });
            notification.error('Failed to load reviews');
        } finally {
            setLoading(false);
        }
    }, [productId, getAllReview, normalizeReviewWithContext, notification]);
    ...
    const getUserInfo = (targetUserId) => {
        if (!targetUserId) return { name: 'Anonymous User', email: 'N/A' };
        const userInfo = getAllUser.find(u => u.id === targetUserId);
    ...
    const handleSubmitReview = async (e) => {
        ...
        try {
            const reviewData = {
                ...
                user_id: userId,
                user_name: userId ? user?.name : reviewForm.anonymousName.trim(),
                user_email: userId ? user?.email : (reviewForm.anonymousEmail.trim() || null),
                is_anonymous_review: !userId,
            ...
            logger.error('Error submitting review:', { error: error.message || error });
    ...
    const markHelpful = async (reviewId) => {
        ...
        try {
            ...
            if (storedVotes) {
                try {
                    votedReviews = JSON.parse(storedVotes);
                } catch {
                    votedReviews = [];
                }
            }
            ...
            localStorage.setItem(storageKey, JSON.stringify([...votedReviews, reviewId]));
            ...
            setReviews((prev) =>
                prev.map((item) =>
                    item.id === reviewId
                        ? {
                              ...item,
                              helpfulCount: data?.helpful_count ?? newHelpfulCount
                          }
                        : item
                )
            );
            ...
            logger.error('Error marking helpful:', { error: error.message || error });
    ...
    const reportReview = async (reviewId) => {
        ...
        try {
            ...
            logger.error('Error reporting review:', { error: error.message || error });
